...
class Bert_v6(BertPreTrainedModel):
    def __init__(self, config, lstm_hidden_size=128, lstm_num_layers=2, gcn_layer=1, mylstm_hidden_size=128, num_decoupling=1):
        ...
        self.mylstm_hidden_size = 2*config.hidden_size
        ...
        self.graph_dim = 2*config.hidden_size
        self.lstm_f = MyLSTM(2*config.hidden_size, self.mylstm_hidden_size, self.graph_dim) 
        self.lstm_b = MyLSTM(2*config.hidden_size, self.mylstm_hidden_size, self.graph_dim)
    ...
    def forward(
        ...
        with_sa_self = torch.cat((cls_rep,sa_self_),2)#(batch_size, num_chioce, 2hidden_size)
        ...
        lstm_output = torch.cat((lstm_out_f, lstm_out_b), dim=2)#(batch_size, num_chioce, 4hidden_size)
        lstm_output = self.drop_lstm(lstm_output)

        target = lstm_output[:,0,:] #(batch_size, 4hidden_size)
        final_lstm_output = lstm_output.repeat(1,1,4) #(batch_size, num_chioce, 4hidden_size *4)
        for i in range(final_lstm_output.size(0)):
            for j in range(final_lstm_output.size(1)):
                ...
                c = a * b
                d = a - b
                final_lstm_output[i,j,:] = torch.cat((a,b,c,d)) #(4hidden_size *4)
